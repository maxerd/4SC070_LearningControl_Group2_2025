...
function [f_jplus1 update] = feedforwardUpdate(SP,t,r_j,e_j,u_j,f_j,Ts)
%OUT:
...
update_ = zeros(size(f_jplus1_));
update_unf = alpha*L_e_j_;

% Apply robustness filter
for ii = 1:ni
    f_jplus1_(:,ii) = filtfilt(num{ii}, den{ii}, f_jplus1_unf(:,ii));
    update_(:,ii) = filtfilt(num{ii}, den{ii}, update_unf(:,ii));
