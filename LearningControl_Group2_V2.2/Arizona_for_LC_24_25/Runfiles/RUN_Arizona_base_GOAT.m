...
%
% Author: Johan Kon, Peter Visser, Maarten van der Hulst,  Date:   July 2023
% modified by Tjeerd Ickenroth and Max van Haren June 2025 
...
optSDIviewer          = true;               % (true/false)                  % Open simulink data inspector
clear_optBuild                                                             

Nrun = 1;                                                                   % Number of trials (one trial is 3 times forth and back)

warning('off')

%% ========================================================================
% ARIZONA OPTIONS - settings that can NOT be changed
% =========================================================================

BadControllers        = false;               % (true/false) if true, uses worse controllers (fbw~4 Hz), nice for demonstration penholder 
...
% Reference options                                                         
optSelectRef          = 'LC24_25';                                             % Set reference.   Change details in 'select_reference.m'
...
% Feedforward options                                                       
optFFmethod           = 'off'               % (off/ILC/other)               % Set Feedforward method. Change details ILC in 'feedforwardUpdate_XXX.m' 
optFFdirections       = [0,0,0]             % (0=OFF, 1=ON)                 % Set direction [y x phi] to apply feedforward
N_trial               = 1;                                                 % Set # trials
...
%% ========================================================================
% Student Pre-Build calculations
% =========================================================================

% If you want to compute anything before building, type your code here.


%% ========================================================================
% Pre-Build calculations: DO NOT TOUCH
% =========================================================================

load('Single_ref_LC24_25.mat')

yref=repmat(r,Nrun,1);
xref = zeros(size(yref));
phiref = zeros(size(yref));
t = 0:Ts:(length(yref)-1)*Ts;

dist = repmat(d,Nrun,1);
vel = repmat(v,Nrun,1);
acc = repmat(a,Nrun,1);

save('../References/Reference_LC24_25','t','yref','xref','phiref')
save('Disturbance_LC24_25','t','dist','vel','acc')
...
%            PenONOFF, PenColor);                                           % [1,2,3,4 = pen positions, 5 = pen up, must be same length as amount of trials]
    ...
    yref = round(yref*optRefDirections(1),6); 
...
history.uhat = NaN(N_trial,Nref,no);                                        % [Trial, time, dim]
...
load Disturbance_LC24_25
history.f(1,:,1) = dist+vel*0.95+acc*0.81;

historyPlot = history;
historyPlot.f(1,:,1) = history.f(1,:,1)-dist';

PlotTrialDataContour(historyPlot,0,1,0,0,1,0,0);                                % Plots initial input
PlotTrialDataContour(historyPlot,1,0,0,0,0,1,0);                                % Plots reference
...
%     end
    ...
    % Extract trial data
    % =====================================================================
    [f_j,u_j,e_j,p1_j,p2_j,uhat_j] = trialData_process(logsOut);                   % p1_j primair encoder data, p2_j secundair encoder data
...
%     history.epsilonNorm(jj) = vecnorm_2016b(epsilon);
    ...
    historyPlot = history;
    historyPlot.f(1,:,1) = history.f(1,:,1)-dist';
%     historyPlot.f(2,:,1) = history.f(2,:,1)-dist';
    PlotTrialDataContour(historyPlot,jj,0,1,0,0,0,0);                           % Increase trial in plot
    PlotTrialDataContour(historyPlot,jj,0,0,0,0,0,1);                           % Plots error and position
...
%     PlotTrialDataContour(historyPlot,jj+1,0,0,0,0,1,0);                       
    ...
    % Update trial data plot
%     PlotTrialDataContour(historyPlot,jj,0,0,0,1,0,0);
